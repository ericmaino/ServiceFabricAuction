...
namespace SFAuction.Svc.Auction
{
    ...
    internal sealed class AuctionSvc : StatefulService
    {
        ...
        {
            return new[] {
             new ServiceReplicaListener(CreateInternalListener, c_replicaEndpoint, false),
             new ServiceReplicaListener(CreateOwinListner, "ApiListener", false)
         };
        }

        private ICommunicationListener CreateOwinListner(StatefulServiceContext arg)
        {
            return new OwinCommunicationListener(StartWebListener, arg, "WebServiceEndpoint");
        }
